...
from utils.utils import (
    ...
    writeRow,
    writeHeader,
    adjlist_to_sparse_matrix
)
from model.model import SSCLayer
...
class ModelHandler(object):
    ...
    def train(self):
        ...
        if args.model == "SSC-GNN":
            if args.cuda:
                gnn_model = SSCLayer(2, inter1, args.alpha, neigh_gen, torch.device('cuda'))
            else:
                gnn_model = SSCLayer(2, inter1, args.alpha, neigh_gen, torch.device("cpu"))
            ...
            for batch in range(num_batches):
                ...
                if epoch <= args.gen_update_end:
                    ...
                    for c_l, e_l, op in zip(ctx_, env_, optimizer_gen):
                        ...
                        env = b_xent(e_l, l)
                        tmpl = (7 * torch.mean(con) + 3*torch.mean(env)) / 10   
                        if i < 2:
